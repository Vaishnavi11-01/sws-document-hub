...
const processDocument = async (document) => {
  ...
  const interval = setInterval(async () => {
    ...
    if (progress >= 100) {
      ...
      if (document.batchId) {
        const batch = activeBatches.get(document.batchId);
        if (batch) {
          batch.completed += 1;
          if (batch.completed >= batch.total) {
            activeBatches.delete(document.batchId);
            const message = `${batch.total} files uploaded successfully`;
            await createNotification({ message, type: "success" });
          } else {
            activeBatches.set(document.batchId, batch);
          }
        }
      }
...
app.post("/api/documents", upload.single("document"), async (req, res) => {
  try {
    ...
    const batchSize = parseInt(req.body.batchSize, 10) || 0;
    ...
    if (batchId && batchSize > 1) {
      const batch = activeBatches.get(batchId) || { total: batchSize, completed: 0 };
